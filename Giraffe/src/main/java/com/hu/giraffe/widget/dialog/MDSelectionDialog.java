...
public class MDSelectionDialog {
    ...
    public MDSelectionDialog(Builder builder) {
        ...
        dialogView = View.inflate(mBuilder.getContext(), R.layout.dialog_md_mid, null);
    ...
    private void loadItem() {
        ...
        if (datas.size() == 1) {
            ...
            button.setBackgroundResource(R.drawable.widget_md_single_selector);
            linearLayout.addView(button);
        } else if (datas.size() > 1) {
            for (int i = 0; i < datas.size(); i++) {
                Button button = getButton(datas.get(i), i);
                if (i == 0) {
                    button.setBackgroundResource(R.drawable.widget_md_top_selector);
                } else if (i > 0 && i != datas.size() - 1) {
                    button.setBackgroundResource(R.drawable.widget_md_middle_selector);
                } else {
                    button.setBackgroundResource(R.drawable.widget_md_bottom_selector);
                }
                linearLayout.addView(button);
            }
        }
    ...
    private Button getButton(String text, int position) {
        ...
        button.setPadding(DensityUtil.dip2px(10), 0, DensityUtil.dip2px(10), 0);

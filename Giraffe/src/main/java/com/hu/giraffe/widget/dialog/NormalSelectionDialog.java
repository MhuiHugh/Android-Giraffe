...
public class NormalSelectionDialog {
    ...
    public NormalSelectionDialog(Builder builder) {
        ...
        dialogView = View.inflate(mBuilder.getContext(), R.layout.dialog_bottom, null);
        mDialog.setContentView(dialogView); // 一定要在setAttributes(lp)之前才有效

        //设置dialog的宽
        Window dialogWindow = mDialog.getWindow();
        WindowManager.LayoutParams lp = dialogWindow.getAttributes();
        lp.width = (int) (DensityUtil.getScreenWidth() *
    ...
    private void loadItem() {
        ...
        if (mBuilder.getTitleVisible()) {
            ...
            l.height = DensityUtil.dip2px(mBuilder.getTitleHeight());
            title.setLayoutParams(l);

            if (datas.size() != 0) {
                title.setBackgroundResource(R.drawable.widget_actiondialog_top_selector);
            } else {
                title.setBackgroundResource(R.drawable.widget_actiondialog_single_selector);
            }
        } else {
        ...
        if (datas.size() == 1) {
            ...
            if (mBuilder.getTitleVisible()) {
                button.setBackgroundResource(R.drawable.widget_actiondialog_bottom_selector);
            } else {
                button.setBackgroundResource(R.drawable.widget_actiondialog_single_selector);
            }
            ...
            for (int i = 0; i < datas.size(); i++) {
                ...
                if (!mBuilder.getTitleVisible() && i == 0) {
                    button.setBackgroundResource(R.drawable.widget_actiondialog_top_selector);
                } else {
                    if (i != datas.size() - 1)
                        button.setBackgroundResource(R.drawable.widget_actiondialog_middle_selector);
                    else
                        button.setBackgroundResource(R.drawable.widget_actiondialog_bottom_selector);
                }
                linearLayout.addView(button);
    ...
    public static class Builder {
        ...
        public Builder(Context context) {
            ...
            titleText = "请选择";
            titleTextColor = ContextCompat.getColor(context, R.color.colorThree);
            titleTextSize = 13;

            onItemListener = null;
            itemHeight = DensityUtil.dip2px(45);//默认item高度
            itemWidth = 0.92f;
            itemTextColor = ContextCompat.getColor(mContext, R.color.colorThree); //默认字体颜色
        ...
        public Builder setItemHeight(int dp) {
            this.itemHeight = DensityUtil.dip2px(dp);
            return this;
        }

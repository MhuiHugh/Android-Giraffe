...
public class SystemUtil {
     ...
     * @param phoneNum
     */
    public static void callPhoneGo(Context cxt, String phoneNum) {
    ...
    /**
     * 判断当前应用程序是否后台运行，使用ForgroundCallbacks监听
     */
    ...
    public static boolean isTopActivy(String cmdName) {
        ActivityManager manager = (ActivityManager) MyApp.getInstance().getSystemService(Activity.ACTIVITY_SERVICE);
        List runningTaskInfos = manager.getRunningTasks(1);
        String cmpNameTemp = null;
        if (null != runningTaskInfos) {
            ActivityManager.RunningTaskInfo one = (ActivityManager.RunningTaskInfo) runningTaskInfos.get(0);
            cmpNameTemp = one.topActivity.toString();
            cmpNameTemp = cmpNameTemp.substring(cmpNameTemp.indexOf("/") + 1, cmpNameTemp.length() - 1);
        }
    ...
    public static int gc(Context cxt) {
            ...
            for (RunningAppProcessInfo process : processList) {
                ...
                if (process.importance > RunningAppProcessInfo.IMPORTANCE_VISIBLE) {
                    ...
                    for (String pkgName : pkgList) {
                        LogUtil.v("======kill pageName：" + pkgName);
                        try {
        ...
        LogUtil.v("清理了" + (getDeviceUsableMemory(cxt) - i) + "M内存");
        return count;
    }
